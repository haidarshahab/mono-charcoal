        ...
        <section className="relative bg-gradient-to-br from-[#1D3F30] via-[#143728] to-[#1D3F30] text-white py-20 lg:py-28">
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto text-center">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6">{t.pages.oem} <span className="block text-accent">{t.oem.charcoalMfg || "Charcoal Manufacturing"}</span></h1>
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <a href="#process" className="bg-accent hover:bg-accent/90 text-white font-semibold px-8 py-3 rounded-lg transition-colors">{t.oem.howItWorks || "How It Works"}</a>
        ...
        <section className="py-20">
          <div className="container mx-auto px-4">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {benefits.map((benefit, index) => (<div key={index} className="bg-white rounded-xl p-8 shadow-lg"><benefit.icon className="w-12 h-12 text-accent mb-4" /><h3 className="text-xl font-semibold text-slate-900 mb-3">{benefit.title}</h3><p className="text-slate-600">{benefit.description}</p></div>))}
            </div>
          </div>
        </section>
        ...
        <section id="process" className="py-20 bg-slate-50">
          <div className="container mx-auto px-4">
            ...
            <div className="grid md:grid-cols-5 gap-4">
              {process.map((p, index) => (<div key={index} className="bg-white rounded-xl p-6 text-center shadow-md"><div className="w-10 h-10 bg-accent text-white rounded-full flex items-center justify-center font-bold mx-auto mb-3">{p.step}</div><h3 className="font-semibold text-slate-900 mb-2">{p.title}</h3><p className="text-slate-600 text-sm">{p.description}</p></div>))}
            </div>
          </div>
        </section>
        ...
        <section className="py-20">
          <div className="container mx-auto px-4">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {packagingOptions.map((pkg, index) => (<div key={index} className="bg-white rounded-xl p-8 shadow-lg"><h3 className="text-xl font-semibold text-slate-900 mb-4">{pkg.category}</h3><div className="mb-4"><p className="text-sm text-slate-500 mb-2">{t.oem.sizes || "Sizes:"}</p><div className="flex flex-wrap gap-2">{pkg.options.map((opt, i) => (<span key={i} className="bg-slate-100 text-slate-700 px-3 py-1 rounded text-sm">{opt}</span>))}</div></div><div><p className="text-sm text-slate-500 mb-2">{t.oem.features || "Features:"}</p><div className="flex flex-wrap gap-2">{pkg.features.map((f, i) => (<span key={i} className="bg-accent/5 text-accent px-3 py-1 rounded text-sm">{f}</span>))}</div></div></div>))}
            </div>
          </div>
        </section>
        ...
        <section className="py-20 bg-slate-900 text-white">
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <div className="flex items-center gap-2 bg-slate-800 px-6 py-3 rounded-lg"><CheckCircle className="w-5 h-5 text-accent" /><span>{t.oem.signedNDA || "Signed NDA"}</span></div>
                <div className="flex items-center gap-2 bg-slate-800 px-6 py-3 rounded-lg"><CheckCircle className="w-5 h-5 text-accent" /><span>{t.oem.dedicatedLine || "Dedicated production line"}</span></div>
                <div className="flex items-center gap-2 bg-slate-800 px-6 py-3 rounded-lg"><CheckCircle className="w-5 h-5 text-accent" /><span>{t.oem.designConf || "Design confidentiality"}</span></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-20">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto bg-gradient-to-br from-accent to-accent rounded-2xl p-8 md:p-12 text-center text-white">
              ...
              <a href="https://wa.me/62881024922133" className="inline-block bg-white text-accent font-semibold px-8 py-3 rounded-lg hover:bg-slate-100 transition-colors">{t.oem.contactOEM || "Contact Us for OEM"}</a>
            </div>
          </div>
        </section>
      </div>
